...
class TestTestRunner:
    def test_single_test_run_with_obstacles_and_car_is_out_of_lane(self, mocker):
        ...
        test_runner.test_loader = mocker.patch(
            "sdc_scissor.testing_api.test_loader.TestLoader"
        )
        test_runner.ml_component = mocker.stub()
        test_runner.simulator = mocker.patch(
        ...
        test_runner.obstacle_factory = mocker.patch(
            "sdc_scissor.obstacle_api.obstacle_factory.ObstacleFactory"
        )
        test_runner.fov = 120
