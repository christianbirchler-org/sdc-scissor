...
class TestPredictor:
    def setup_class(self):
        self.test_dir = (
            Path(os.path.dirname(__file__)).parent.parent.parent / "sample_tests"
        )
        self.test_data_csv_path = (
            Path(os.path.dirname(__file__)) / "test_road_features.csv"
        )

    def test_prediction_with_train_data(self, fs):
        fs_test_dir = Path("/sample_tests")
        fs.add_real_directory(
            source_path=self.test_dir, read_only=False, target_path=fs_test_dir
        )
        fs.add_real_file(
            source_path=self.test_data_csv_path,
            target_path=fs_test_dir / "test_road_features.csv",
        )
        ...
        test_loader = TestLoader(
            tests_dir=fs_test_dir, test_validator=SimpleTestValidator()
        )
        ...
        test_predictor = Predictor(
            ...
            joblib_classifier=Path("models/decision_tree.joblib"),
        )
