...
def _plot_road_segments(segments, fig=None, title=None):
    ...
    for segment_id, segment in enumerate(segments):
        if segment["type"] == "straight":
            # s = 's'
            c = color_map_straights[segment_id % len(color_map_straights)]
        else:
            c = color_map_turn[segment_id % len(color_map_turn)]
            # s = 'o'

        c = _adjust_lightness(c, lightness[segment_id])
        ...
        plt.plot(*sector_poly.exterior.xy, marker='.', color=c)  # pylint: disable=no-member
...
class RoadTestEvaluatorTest(unittest.TestCase):
    ...
    @staticmethod
    def _plot_oob(oob_pos, segment_before, segment_after):
    ...
    def test_obe(self):
        ...
        oob_pos, segment_before, segment_after, _oob_side = road_test_evaluation.\
